...
extern void wifi_dsm_report_info(int dsm_id, const char *fmt, ...);
...
#ifdef CONFIG_HUAWEI_DSM
		wifi_dsm_report_info(DSM_WIFI_OPEN_ERROR, "result:%d error:%d",resp.resp.result, resp.resp.error);
#endif
		goto out;
	}
...
#ifdef CONFIG_HUAWEI_DSM
			wifi_dsm_report_info(DSM_WIFI_SSR_ERROR, "pd_state: %d, state: 0x%lx: cause: %s", *state, priv->state, icnss_pdr_cause[cause]);
#endif
		}
		break;
	default:
