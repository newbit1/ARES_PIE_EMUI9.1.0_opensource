...
static const char subsys_modem_name[] = "modem";
...
#ifdef CONFIG_PIL_Q6V5_MSS
        if (fw_name && strncmp(fw_name, subsys_modem_name, (sizeof(subsys_modem_name) - 1)) == 0) {
...
#ifdef CONFIG_PIL_Q6V5_MSS
        if(subsystem && strncmp(subsys->desc->name, subsys_modem_name,
                                (sizeof(subsys_modem_name) - 1)) == 0) {
            printk("PIL: %s called by %s\n", __func__, current->comm);
            WARN_ON(1);
        }
#endif
