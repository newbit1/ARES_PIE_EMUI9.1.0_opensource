...
static  struct power_supply_desc battery_main_psy_desc = {
	.name = "Battery",
...
static int hw_init_batt_main_psy(struct huawei_battery_info *info)
{
	struct power_supply_config batt_cfg = {};
	int rc = 0;
	...
	batt_cfg.drv_data = info;
	batt_cfg.of_node = info->dev->of_node;
	battery_main_psy_desc.num_properties = info->num_props;
	battery_main_psy_desc.properties =info->batt_props;
	info->batt_psy = power_supply_register(info->dev,
		(const struct power_supply_desc*) (&battery_main_psy_desc), &batt_cfg);
...
static int huawei_battery_probe(struct platform_device *pdev)
{
   ...
   ret = hw_init_batt_psy(info);
    ...
    /* Copy the sysfs nodes form battery to Battery */
    ret = hw_init_batt_main_psy(info);
    if (ret < 0) {
        pr_err("Couldn't register Battery power supply\n");
        goto PSY_REG_FAIL;
    }
