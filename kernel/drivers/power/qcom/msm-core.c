...
static void set_threshold(struct cpu_activity_info *cpu_node)
{
	 ...
	 */
	ret = sensor_activate_trip(cpu_node->sensor_id,
			&cpu_node->hi_threshold, false);
	if (ret != 0) {
		pr_err("sensor:%u Error in disable hi_threshold. err:%d\n",
			cpu_node->sensor_id, ret);
	}
	ret = sensor_activate_trip(cpu_node->sensor_id,
			&cpu_node->low_threshold, false);
	if (ret != 0) {
		pr_err("sensor:%u Error in disable low_threshold. err:%d\n",
			cpu_node->sensor_id, ret);
	}
...
void trigger_cpu_pwr_stats_calc(void)
{
	...
	for_each_online_cpu(cpu) {
		...
		if (cpu_node->temp == prev_temp[cpu]) {
			...
			if (rc < 0) {
				pr_err("Unable to read cpu sensor::%d\n",
					 cpu_node->sensor_id);
			}
				...
				unsigned long val, void *data)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		for_each_possible_cpu(cpu) {
			...
			ret = sensor_activate_trip(activity[cpu].sensor_id,
				&activity[cpu].hi_threshold, false);
			if (ret != 0) {
				pr_err("sensor:%u Error in disable hi_threshold. err:%d\n",
					activity[cpu].sensor_id,ret);
			}
			ret = sensor_activate_trip(activity[cpu].sensor_id,
