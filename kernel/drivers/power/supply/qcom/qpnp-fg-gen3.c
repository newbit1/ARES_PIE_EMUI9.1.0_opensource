...
static int fg_gen3_debug_mask = FG_IRQ;
module_param_named(
...
static int fg_get_debug_batt_id(struct fg_chip *chip, int *batt_id)
{
	 ...
	 */
	temp = (u64)(buf[1] << 8 | buf[0]) * 2500000;
...
static int fg_load_learned_cap_from_sram(struct fg_chip *chip)
{
	...
	chip->cl.learned_cc_uah = (int64_t)act_cap_mah * 1000;
...
static void fg_cap_learning_post_process(struct fg_chip *chip)
{
	...
	pr_info("[%s]:final cc_uah = %lld, learned capacity %lld -> %lld uah\n",
		__func__,chip->cl.final_cc_uah, old_cap, chip->cl.learned_cc_uah);
#ifdef CONFIG_HUAWEI_PMU_DSM
...
static int fg_cap_learning_process_full_data(struct fg_chip *chip)
{
	...
	pr_info("[%s]:Current cc_soc=%d cc_soc_delta_pct=%d total_cc_uah=%lld\n",
		__func__,cc_soc_sw, cc_soc_delta_pct, chip->cl.final_cc_uah);
	return 0;
}
...
static int fg_cap_learning_begin(struct fg_chip *chip, u32 batt_soc)
{
	...
	pr_info("[%s]:Capacity learning started @ battery SOC %d init_cc_soc_sw:%d\n",
		__func__,batt_soc_msb, chip->cl.init_cc_soc_sw);
out:
	return rc;
}
...
static int fg_rconn_config(struct fg_chip *chip)
{
	...
	scaling_factor = div64_u64((u64)esr_uohms * 1000,
				esr_uohms + (u64)(chip->dt.rconn_mohms * 1000));
...
static int fg_set_constant_chg_voltage(struct fg_chip *chip, int volt_uv)
{
	...
	pr_info("[s%]:set constant_chg_voltage to %d\n",__func__, volt_uv);
	return 0;
}
...
static int fg_set_recharge_soc(struct fg_chip *chip, int recharge_soc)
{
	...
	pr_info("[s%]:set recharge_soc to %d\n",__func__, recharge_soc);
	return 0;
}
...
static int fg_adjust_recharge_soc(struct fg_chip *chip)
{
	...
	pr_info("[%s]:resume soc set to %d\n",__func__, new_recharge_soc);
	return 0;
}
...
static void fg_reset_learned_cc(struct fg_chip *chip, int clear)
{
	...
	rc = fg_save_learned_cap_to_sram(chip);
	if (rc < 0)
		pr_err("Error in reset eaving learned_cc_uah, rc=%d\n", rc);
}
...
static void qpnp_fuelguage_dump_register(char *reg_value)
{
	...
	snprintf(reg_value, MAX_SIZE, "%-8d    %-12lld    %-8d    %-14lld    %-7d    %-10d    %-11d    %-14d    %-11lld    %-11lld    %-10lld    %-14d    ",
