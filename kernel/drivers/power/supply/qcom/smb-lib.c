...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB:
		...
		if (!is_client_vote_enabled(chg->usb_icl_votable,
			...
			smblib_err(chg, "USB vote icl 500mA !\n");
		}
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, false, 0);
		break;
	case POWER_SUPPLY_TYPE_USB_CDP:
		if (chg->current_cdp_ua < 0) {
			...
			smblib_err(chg,"USB_CDP vote icl 1500mA !\n");
		} else {
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, chg->current_cdp_ua);
			smblib_err(chg,"USB_CDP vote icl %d!\n",chg->current_cdp_ua);
		}
		break;
	case POWER_SUPPLY_TYPE_USB_DCP:
		...
		smblib_err(chg, "USB_DCP vote icl %d\n", rp_ua);
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		...
		if (chg->current_float_ua < 0) {
			...
			smblib_err(chg, "USB_FLOAT vote icl 500mA!\n");
		} else {
			vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, chg->current_float_ua);
			smblib_err(chg, "USB_FLOAT vote icl %d!\n",chg->current_float_ua);
		}
		break;
	case POWER_SUPPLY_TYPE_USB_HVDCP:
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
		if (chg->current_hvdcp_ua < 0) {
			...
			smblib_err(chg, "USB_HVDCP vote icl 3000mA!\n");
		} else {
...
static void typec_sink_removal(struct smb_charger *chg)
{
	...
	if (rc < 0)
		smblib_err(chg,
			"Error in typec sink_removal rc=%d\n", rc);
...
static void smblib_otg_oc_work(struct work_struct *work)
{
	...
	if (rc < 0) {
		smblib_err(chg,
			"Couldn't disabling VBUS < 1V check rc=%d\n", rc);
	}
