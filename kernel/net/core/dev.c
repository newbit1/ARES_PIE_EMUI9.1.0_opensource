...
#define MaxRefcntErrorTimes  6
extern int refcntErrorTimes;
...
static void netdev_wait_allrefs(struct net_device *dev)
{
	...
	while (refcnt != 0) {
		...
		if (time_after(jiffies, warning_time + 10 * HZ)) {
			...
			{
				refcntErrorTimes++;
				if(refcntErrorTimes == MaxRefcntErrorTimes)
				{
					wifi_dsm_report_info(DSM_WIFI_MODULE_INIT_ERROR, "refCount fail: Usage count = %d", refcnt);
				}
			}
#endif
		}
