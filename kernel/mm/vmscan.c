...
static inline int is_page_cache_freeable(struct page *page, struct address_space *mapping)
{
...
#define TRY_TO_UNMAP(_page, _ttu_flags, target_vma)				\
	do {							\
		switch (try_to_unmap(_page, _ttu_flags, target_vma)) {	\
		case SWAP_FAIL:					\
				      ...
				      bool force_reclaim)
{
	...
	while (!list_empty(page_list)) {
		...
		ttu_flags = (ttu_flags | TTU_BATCH_FLUSH);
