	...
	unsigned playload_pages)
{
	...
	memset(playload, 0, playload_pages * PAGE_SIZE);
	return playload;
}
	...
	struct s_ws_data *data, unsigned playload_pages, unsigned *pidx)
{
	...
	if (data->array_page_cnt) {
		...
		memcpy(page_array, data->page_array, cpy_size);
	}
	...
	struct s_ws_owner *owner)
{
	...
	memcpy(record->owner.name, owner->name, len);

	len = strlen(owner->record_path) + 1;
	memcpy(record->owner.record_path, owner->record_path, len);
	...
	struct file *file, struct s_file_info **file_info, bool *is_existed)
{
	...
	if (!existed) {
		...
		strncpy(fileinfo->path_node.path,
			filepath, path_len + 1);
	...
	struct s_ws_record *record, bool is_exist)
{
	...
	if (!is_exist) {
		...
		memcpy(record->owner.name, ws->owner.name, cpy_size);

		cpy_size = strlen(ws->owner.record_path) + 1;
		memcpy(record->owner.record_path, ws->owner.record_path, cpy_size);
	}
