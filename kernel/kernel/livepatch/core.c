...
static int do_upload_log(long type, const char *desc)
{
	...
	if (obj) {
		...
		ret += imonitor_set_param(obj, E940000002_ERRDESC_VARCHAR,
					  (long)desc);
		if (ret) {
...
static int klp_init(void)
{
	...
	res = proc_create("livepatch/state", 0,
			  NULL, &proc_klpstate_operations);
	if (!res)
		goto error_remove;
...
error_remove:
	...
	(void)do_upload_log(UPLOADTYPE_INIT_FAIL,
			    "kernel patch module initialize failed.");
error_out:
	return -ENOMEM;
}
