				...
				union afe_spkr_prot_config *prot_config)
{
	...
	case AFE_TFA_SET_BYPASS:
		param_info.module_id = AFE_MODULE_TFA_RX;
		break;
#endif /*CONFIG_HUAWEI_SMARTPAKIT_AUDIO*/
	default:
...
int send_tfa_cal_apr(void *buf, int cmd_size, bool bRead)
{
	...
	if (!bRead) {
		...
		pr_err("%s:afe_set_apr_msg is %x!\n", __func__, afe_set_apr_msg);
		...
		pr_err("%s:afe_set_apr_msg addr is %x %x!\n", __func__, afe_set_apr_msg->mem_hdr.data_payload_addr_lsw, afe_set_apr_msg->mem_hdr.data_payload_addr_msw);
...
int send_tfa_cal_in_band(void *buf, int cmd_size, int param_id)
{
	union afe_spkr_prot_config afe_spk_config = {0};
	int ret = 0, port_id = AFE_PORT_ID_TERTIARY_MI2S_RX;
	pr_err("%s:enter!\n",__func__);
