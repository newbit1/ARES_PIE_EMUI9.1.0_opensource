				...
				int discard_type, unsigned int granularity)
{
	if (discard_type == DPOLICY_BG) {
		*dpolicy = dpolicys[DPOLICY_BG];
	} else if (discard_type == DPOLICY_BL) {
		*dpolicy = dpolicys[DPOLICY_BL];
	} else if (discard_type == DPOLICY_FORCE) {
		*dpolicy = dpolicys[DPOLICY_FORCE];
	} else if (discard_type == DPOLICY_FSTRIM) {
		*dpolicy = dpolicys[DPOLICY_FSTRIM];
		/* min_len receive from user */
		if (dpolicy->granularity != granularity)
			dpolicy->granularity = granularity;
	} else if (discard_type == DPOLICY_UMOUNT) {
		*dpolicy = dpolicys[DPOLICY_UMOUNT];
	}
}
...
#ifdef CONFIG_F2FS_JOURNAL_APPEND
	if (nats_in_cursum(nat_j) > (NAT_JOURNAL_ENTRIES + NAT_APPEND_JOURNAL_ENTRIES) ||
			sits_in_cursum(sit_j) > (SIT_JOURNAL_ENTRIES + SIT_APPEND_JOURNAL_ENTRIES))
#else
