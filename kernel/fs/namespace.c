...
#ifdef CONFIG_HW_BFMR_QCOM
	if ((retval == 0) &&
		(copy_from_user(bfmr_umount_name, name, BFMR_MOUNT_NAME_SIZE-1) == 0)) {
...
#ifdef CONFIG_HW_BFMR_QCOM
	if ((retval == 0) &&
		(copy_from_user(bfmr_mount_name, dir_name, BFMR_MOUNT_NAME_SIZE-1) == 0)) {
